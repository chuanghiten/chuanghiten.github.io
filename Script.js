...
fdiv.onmousedown = function(e) {
  ...
  posY = 106/2 /*e.clientY - parseInt(fdiv.style.top)*/;
...
const ButtonScollToTop = document.querySelector("#ItemMenu");
let menuOpen = false;
...
ButtonScollToTop.addEventListener("click", () => {
  if (!menuOpen) {} else {
    buttonMenu.classList.remove("open");
    contentContainer.classList.remove("open");
    body.classList.remove("open");
...
$(document).ready(function() {
  $(".ButtonScollToTop").click(function(event) {
    event.preventDefault();
    part = $(this).attr("href");
